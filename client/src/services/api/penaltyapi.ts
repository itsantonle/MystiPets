...
export const getUserPenalties = async (player_uuid: string) => {
  return (await penaltyAPI.get(`/getUserPenalty/${player_uuid}`)).data
    .data[0]
}

export const getPenalties = async (penalty_id: Number) => {
  return (await penaltyAPI.get(`/getPenalty/${penalty_id}`)).data
    .data[0]
}
...
export const assignPenalty = async (
  ...
  await penaltyAPI.put(`/assignPenalty/${player_uuid}`, {
    player_penalty,
  })
}
...
export const updatePenaltyType = async (
  ...
  await penaltyAPI.put(`/updatePenaltyType/${penalty_id}`, {
    penalty_type,
  })
}
...
export const updatePenaltyDuration = async (
  ...
  await penaltyAPI.put(`/updatePenaltyDuration/${penalty_id}`, {
    penalty_duration,
  })
}
